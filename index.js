...
// ─── bizhawk tcp socket ───────────────────────────────────────────────────────
let bizhawkSocket = null;

net.createServer((socket) => {
    console.log("BizHawk connected");
    socket.setNoDelay(true);
    bizhawkSocket = socket;
    socket.on("end", () => {
        console.log("BizHawk disconnected");
        bizhawkSocket = null;
    });
}).listen(8888, "127.0.0.1", () => console.log("Waiting for BizHawk on port 8888"));

function sendToBizhawk(button) {
    if (bizhawkSocket) {
        bizhawkSocket.write(button + "\n");
        console.log("→ bizhawk:", button);
    } else {
        console.warn("BizHawk not connected, dropping:", button);
    }
}
